### 📊 Ride Analytics Dashboard (Simulated OLA Case Study)

This project showcases a simulated ride-hailing analytics dashboard built using **Power BI** and dummy data to replicate real-world business scenarios. It provides insights into bookings, vehicle performance, revenue, cancellations, and customer satisfaction.

--

### 🛠️ Tech Stack
- Power BI  
- Excel  
- DAX  

---

### 🌐 Data Source
- Dummy datasets created for educational and reporting purposes  
- Includes booking logs, vehicle types, payment methods, cancellation reasons, and ratings  
- Structured in Excel and modeled in Power BI  

---

### ✨ Features
- Overall booking summary with status breakdown and daily trends  
- Vehicle type analysis: success rates, booking value, and average distance  
- Revenue insights: payment method preferences and top customer contributions  
- Cancellation metrics: behavioral breakdown by customers and drivers  
- Ratings overview: driver and customer scores segmented by vehicle type  
- Interactive visuals: slicers, drill-throughs, tooltips, and dynamic charts  

---

### 📁 Files
- `RideAnalyticsDashboard.pbix`: Power BI report  
- `dashboard_demo.mp4`: Demo walkthrough (optional)  
- `README.md`: Project documentation  

---

